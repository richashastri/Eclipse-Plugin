...
public class IfStartCompartmentFigureListener implements FigureListener {
	...
	public static final double R = 100; // The base length
	...
	public IfStartCompartmentFigureListener(ListCompartmentEditPart compartmentEditPart, RoundedRectangle roundedRectangle) {
	...
	public void figureMoved(IFigure f) {
		...
		if (figure.getSize().width != 0) {
			...
			boolean firstOutputProcessed = false;
				...
				if (child instanceof AbstractGraphicalEditPart) {
					...
					if (gEditPart instanceof OutputPortEditPart) {
						constraint = new Rectangle(
								...
								!firstOutputProcessed ? (int) ((MARGIN + R) * yScale) : (int) ((MARGIN + 3 * R) * yScale),
								(int) (R * xScale), 
								(int) (R * yScale));
						contentPane.setConstraint(gEditPart.getFigure(), constraint);
						firstOutputProcessed = true;
					}
