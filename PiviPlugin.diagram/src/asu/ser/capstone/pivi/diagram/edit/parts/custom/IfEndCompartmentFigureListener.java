...
public class IfEndCompartmentFigureListener implements FigureListener {
	...
	public IfEndCompartmentFigureListener(ListCompartmentEditPart compartmentEditPart, RoundedRectangle roundedRectangle) {
			...
			for (AbstractEditPart child : childs) {
				if (child instanceof AbstractGraphicalEditPart) {
					...
					if (gEditPart instanceof OutputPortEditPart) {
						...
						constraint = new Rectangle(
								(int) (MARGIN * xScale),
								!firstInputProcessed ? (int) ((MARGIN + R) * yScale) : (int) ((MARGIN + 3 * R) * yScale),
								(int) (R * xScale), 
								(int) (R * yScale));
						contentPane.setConstraint(gEditPart.getFigure(), constraint);
